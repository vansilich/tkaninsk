<?$inc_path = "../../admin/"; $root_path="../../" ;include($inc_path."class/header.php");$this_page_id = 12;	$q = new query();
			$site_pages = new pages($prefix.'pages',$main_page='Главная', $main_page_title = 'index.php');?><? $this_block_id = 18;?>
<?
...
if(empty($cpu)){
	...
	$data = $q->select("SELECT * FROM ".$prefix."staff WHERE status=1 order by created desc  LIMIT ".(get_param($page_name,0)*$page_size).", ".$page_size);
	$numb = $q->select1("select count(id) as number from ".$prefix."staff where status=1");
	$cnt = count($data);
	$total_number = $numb['number'];


	$tmpl = file_get_contents($inc_path.'protoblocks/staff/templates/news.php');
	...
	$row = $q->select1("SELECT * FROM ".$prefix."staff WHERE status=1 AND
	cpu=".to_sql($cpu));
	$date = date('d.m.Y',to_phpdate($row['created']));

	$tmpl_row = file_get_contents($inc_path.'protoblocks/staff/templates/news_full.php');
	$tmpl_row = str_replace('{NEWS_NAME}',$row['name'],$tmpl_row);
	$tmpl_row = str_replace('{NEWS_ANONS}',$row['anons'],$tmpl_row);
	...
	echo '<div><a href="'.$link.'">Назад</a></div>';
}
