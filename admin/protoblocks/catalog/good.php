<?
 ...
 if(!empty($color)){
	$this_color = $q->select1("select G.*,C.code,C.name as cname from ".$prefix."goods as G
	left join ".$prefix."color as C on C.id = G.color
	where groups>0 and groups=".to_sql($row['groups'])." and G.id=".to_sql($color));
	if(!empty($this_color)){
...
$good_colors = $q->select("select G.*,C.name as cname from ".$prefix."goods as G
	left join ".$prefix."adv_params_value as C on C.id = G.color
	where groups>0 and groups=".to_sql($row['groups'])." and kol>0.5");
	/* селектор цветов*/
  $sel_color  = '';
  if(!empty($good_colors)){
	 ...
	 foreach($good_colors as $c){
		 if($c['cname'] == $main_color['cname']) continue;
		$sel_color .= '<option value="/goods/'.$c['cpu'].'/"';
  ...
  for($i=1; $i<=6; $i++){
    ...
    if(!empty($img)){
      ...
      $big_i .= '<a href="'.$img.'?v='.$row['renew'].'"  id="_img'.$i.'" class="products-group';
	  if($flag > 0) $big_i .= ' none';
	  $big_i .= '"><img src="'.$img2.'?v='.$row['renew'].'" alt="'.htmlspecialchars($alt).'" class="img"/></a>
      ';
      $small_i .= '
      <div class="item">
      <img src="'.$img2.'?v='.$row['renew'].'" data-id="_img'.$i.'" width="97" height="97" class="js-smfoto foto_sm" alt=""/>
      </div>';
	  $flag++;
    }
  }
  
  if(empty($big_i)) $big_i = '<img src="/img/nophoto.jpg" alt="'.htmlspecialchars($alt).'" class="foto"/>';
...
where good_id=".to_sql($row['id'])." and types!='nazn' and P.hideit=0 order by P.name ");
...
if(!empty($row['video_content'])){
		$news_row = str_replace('{GOOD_VIDEO_TAB}','<input id="tab_video" type="radio" name="tabs">
<label for="tab_video" title="Видео">Видео</label>',$news_row );
		$news_row = str_replace('{GOOD_VIDEO}','
		<section id="content-tab_video">
		<div align="center" class="videoWrapper">'.$row['video_content'].'</div>
		</section>
		',$news_row );
	}else{
		
		$news_row = str_replace('{GOOD_VIDEO}','',$news_row );
		$news_row = str_replace('{GOOD_VIDEO_TAB}','',$news_row );
	}
...
echo $news_row;
				...
				$sql = "select G.*,C.code,C.name as cname from ".$prefix."goods as G
				left join ".$prefix."color as C on C.id = G.color
				where groups>0 and groups=".to_sql($row['groups'])." and kol>=0.5 and G.id !=".to_sql($row['id']);
				//echo $sql;
