...
function check_title($id,$name){
	...
	$name = trim($name);
	$q = new query();
	$check = $q->select1("select cpu,seo_title from ".$prefix."news where id=".to_sql($id));
	if(empty($check['seo_title'])){
		$q->exec("update ".$prefix."news set seo_title=".to_sql($name)." where id=".to_sql($id));			
	}
	if(empty($check['cpu'])){
		$new_cpu = CleanFileName(translit($name));
		$check = $q->select1("select id from ".$prefix."news where cpu=".to_sql($new_cpu)." and id<>".to_sql($id));
		if($check == 0){
			$q->exec("update ".$prefix."news set cpu=".to_sql($new_cpu)." where id=".to_sql($id));
		}else{
			$new_cpu .= $id;
			$q->exec("update ".$prefix."news set cpu=".to_sql($new_cpu)." where id=".to_sql($id));		
		}	
...
$table = new cTable('Новости',$prefix.'news' ,'id',$is_rank=1, $is_status=1, $is_add=1,$is_edit=1,$is_del=1);
...
<div class="tab-pane active" id="tab_text" style="position: relative; padding:20px; ">'));
	...
	$table->insertcol(new c_image('Картинка','img',1,1,$inc_path.'../files/news/', '145;'));
